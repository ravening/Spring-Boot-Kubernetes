...
public class StartPageController implements ApplicationListener<WebServerInitializedEvent> {
	...
	public Mono<Startpage> getStartpage() {
		...
		Mono<ProductResponse> booksBestSeller = client.get().uri("/books/bestseller").exchange().flatMap(responseProcessor)
				.onErrorResume(t -> {
					t.printStackTrace();
					return Mono.just(errorResponse);
				});

		Mono<Startpage> page = Mono.zip(hotdeals, fashionBestSellers, toysBestSellers, booksBestSeller).flatMap(t -> {
			Startpage p = new Startpage();
			ProductResponse deals = t.getT1();
			ProductResponse fashion = t.getT2();
			ProductResponse toys = t.getT3();
			ProductResponse books = t.getT4();
			p.setFashionResponse(fashion);
			p.setHotDealsResponse(deals);
			p.setToysResponse(toys);
			p.setBooksResponse(books);
			...
			p.setStatusBooks(books.getResponseType().name());
			// Request duration
